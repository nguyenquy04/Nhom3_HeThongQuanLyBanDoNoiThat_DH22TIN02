    ...
    internal class DataNhanVien
    {
        ...
        public static DataTable GetDanhSachNhanVien()
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    string query = "SELECT MaNV, TenNV, NgaySinh, GioiTinh, DiaChi, SoDienThoai, TenDangNhap, MatKhau, ChucVu, NgayTuyenDung FROM NhanVien ORDER BY MaNV DESC";
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        conn.Open();
